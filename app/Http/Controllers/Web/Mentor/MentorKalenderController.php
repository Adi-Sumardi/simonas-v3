        ...
        try {
            ...
            $hafalanEvents = HafalanLog::whereIn('user_id', $menteeIds)
                ...
                ->map(fn($log) => [
                    ...
                    'date'   => $log->tested_at?->format('Y-m-d') ?? $log->created_at?->format('Y-m-d') ?? $today,
                    'time'   => $log->tested_at?->format('H:i') ?? $log->created_at?->format('H:i') ?? '08:00',
                    'type'   => 'hafalan',
                    'color'  => $log->score === 'pending' ? '#f59e0b' : '#10b981',
                    'warga'  => $log->user?->name ?? 'Warga',
            ...
            $kegiatanEvents = Kegiatan::orderByDesc('id')
                ...
                ->map(function ($k) use ($today) {
                    $date = $k->created_at?->format('Y-m-d') ?? $today;
                    $time = '10:00';
                    if (!empty($k->waktu)) {
                        try {
            ...
            $events = $hafalanEvents->concat($kegiatanEvents)->sortBy('date')->values()->toArray();

            $upcoming = collect($events)
                ->filter(fn($e) => isset($e['date']) && $e['date'] >= $today)
                ->sortBy('date')
                ->values()
                ->take(5)
                ->toArray();
            ...
            \Illuminate\Support\Facades\Log::error('MentorKalender error: ' . $e->getMessage() . "\n" . $e->getTraceAsString());
            $events = [];
            $upcoming = [];
            $jadwal_setoran = [];
        }
