...
// Jumlah ayat per surah (indeks 1-114, urutan sama dengan SURAHS) — dipakai untuk
// memvalidasi input ayat mulai/selesai supaya tidak melebihi ayat asli surah tsb.
const SURAH_AYAT_COUNT: Record<number, number> = {
...
function LogModal({ editItem, onClose }: { editItem: HafalanLog | null; onClose: () => void }) {
    ...
    function handleSurahChange(val: string) {
        ...
        const page = idx !== -1 ? SURAH_START_PAGES[idx + 1] : undefined;
        setForm(p => ({
            ...p,
            surah: val,
            ayat_start: 1,
            ayat_end: 1,
            ...(page ? { halaman_start: page, halaman_end: page } : {}),
        }));
    }

    function set(f: string, v: string | number) { setForm(p => ({ ...p, [f]: v })); }

    const selectedSurahIdx = SURAHS.indexOf(form.surah);
    const maxAyat = selectedSurahIdx !== -1 ? SURAH_AYAT_COUNT[selectedSurahIdx + 1] : undefined;
            ...
            <form id="hafalan-form" onSubmit={submit} className="space-y-4">
                <div>
                    <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant mb-1.5 block">
                        Surah *{maxAyat ? <span className="normal-case font-semibold text-on-surface-variant/70"> — {maxAyat} ayat</span> : null}
                    </label>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" min={1} max={maxAyat} value={form.ayat_start}
                            onChange={e => {
                                const v = Math.max(1, Math.min(parseInt(e.target.value) || 1, maxAyat ?? Infinity));
                                set('ayat_start', v);
                                if (form.ayat_end < v) set('ayat_end', v);
                            }}
                    ...
                    <div>
                        ...
                        <input type="number" min={form.ayat_start} max={maxAyat} value={form.ayat_end}
                            onChange={e => set('ayat_end', Math.max(form.ayat_start, Math.min(parseInt(e.target.value) || form.ayat_start, maxAyat ?? Infinity)))}
                            className="glass-input w-full text-sm" required />
                    </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
