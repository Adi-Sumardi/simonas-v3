...
const STEPS = [
    ...
    { key: 'tour', label: 'Kenalan Fitur', icon: 'explore' },
];

const FEATURES_BY_ROLE: Record<UserData['role'], { icon: string; title: string; desc: string }[]> = {
    mahasiswa: [
...
export default function Onboarding({ user }: Props) {
    const FEATURES = FEATURES_BY_ROLE[user.role] ?? FEATURES_BY_ROLE.mahasiswa;
        ...
        <AuthLayout>
            ...
            <div className="glass-panel w-full max-w-lg p-8">
                ...
                <form onSubmit={submit} className="space-y-5">
                    ...
                    {step === 3 && (
                        <div>
                            <p className="text-body-sm text-on-surface-variant text-center mb-5">
                                Yuk kenalan dulu sama menu utama yang bakal sering kamu pakai:
                            </p>
                            <div className="space-y-3">
                                {FEATURES.map(f => (
                                    <div key={f.title} className="flex items-start gap-3 p-3 rounded-xl bg-surface-container/50">
                                        <div className="w-10 h-10 rounded-xl bg-primary-container flex items-center justify-center flex-shrink-0">
                                            <Icon name={f.icon} className="text-xl text-white" filled />
                                        </div>
                                        <div>
